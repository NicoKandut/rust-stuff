...
pub(crate) type Size = f32;
pub(crate) type Point = [f32; 3];
type NodeId = usize;
type Children = [Node; 8];
...
impl Octree {
    ...
    pub(crate) fn split_node(&mut self, node_id: NodeId) -> Result<usize, ()> {
    ...
    pub(crate) fn set(&mut self, node_id: NodeId, data: Material) -> Result<(), ()> {
        if let Some(node) = self.arena.get_mut(node_id) {
            node.data = data;
            Ok(())
        } else {
    ...
    pub(crate) fn set_at(&mut self, target: Point, mat: Material) {}
}
...
pub(crate) fn create_children(node: &Node, node_id: usize) -> [Node; 8] {
    ...
    let children = child_dirs
        ...
        .map(|center| Node::new_child_of(parent, data.clone(), center, half_size));
    children
}
