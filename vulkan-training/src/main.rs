use crate::matrix::get_perspective_matrices;
use crate::matrix::Cross;
...
#[derive(Clone, Debug, Copy)]
struct Camera {
  pos: [f64; 3],   // 3d coordinates
  angle: [f64; 2], // sideways and up/down
}
...
fn main() {
  ...
  let view_projection: M4x4<f64> = [
    [1.0, 0.0, 0.0, cam.pos[0]],
    [0.0, 1.0, 0.0, cam.pos[1]],
    [0.0, 0.0, 1.0, cam.pos[2]],
    [0.0, 0.0, 0.0, 1.0],
  ];
  let (perspective_projection, perspective_inverse) =
    get_perspective_matrices(90.0, WINDOW_WIDTH as f64 / WINDOW_HEIGHT as f64, 2.0, 100.0);
  let cubes = [Cube {
    pos: [0.0, 0.0, 0.0],
  ...
  unsafe {
    let base = AshApp::new(WINDOW_WIDTH, WINDOW_HEIGHT);
